...
(function () {
  ...
  function onHashChangeEvent(event) {
    rememberVerticalScrollPosition(
      getPage(event.oldURL),
  ...
  function showError(error) {
    ...
    try {
      ...
      error(
        `Failed to abort and/or reset the AbortController ${controller} and signal ${signal}.`
      );
      errorCritical();
    }
    ...
    showContent(
      `<div class="card m-6"> <div class="card-content"> <div class="media"> <div class="media-content"> <p class="title is-4">OOPS!</p> </div> </div> <div class="content"> <p>Looks like something went wrong!</p> <p id="error">Don't worry, here's the error:</p></div> </div> </div>`,
      false
    );
  ...
  function showContent(html, append) {
    ...
    try {
      ...
      /* An error here means that the document html has been corrupted.
            ...
            method. */
      error(
        `Failed to show content with html ${html}, and append ${append}. ${e}.`
      );
      errorCritical();
    }
  }
  ...
  function scrollToY(page, history) {
    ...
    try {
      ...
      error(
        `Failed to scroll to Y on page ${page}, with history ${history}. ${e}.`
      );
    }
  }
  ...
  function createHomeItem(html, url) {
    ...
    try {
      ...
      error(
        `Failed to create home item with html ${html}, and url ${url}. ${e}.`
      );
    }
  ...
  function rememberVerticalScrollPosition(page, lastVerticalScrollPosition) {
    ...
    try {
      ...
      error(
        `Failed to set last vertical scroll position with value of ${lastVerticalScrollPosition} at page ${page}. ${e}.`
      );
    }
  }
  ...
  function getVerticalScrollPosition(lastVerticalScrollPosition) {
    ...
    try {
      ...
      error(
        `Failed to get last vertical scroll position with ${lastVerticalScrollPosition} at scrollMaxY value of ${window.scrollMaxY}. ${e}.`
      );
    }
  ...
  function setThemeButtonText() {
    try {
      ...
      error(
        `Failed to set themem buttion text with theme ${theme} and theme button ${document.getElementById(
          "theme"
        )}. ${e}.`
      );
    }
  ...
  function errorParam(expected, actual, param, method) {
    console.error(
      `Unexpected parameter ${param} in method ${method}. Expected ${expected}, got ${typeof actual}.`
    );
  }
